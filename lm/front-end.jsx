...
// 5.1 step aside
//  - XSS - cross site scripting. внедрение вредоносного кода в страницу пользователя.
//      бывают 3х типов:
//        - хранимый(сохраняются в базе данных а потом загружаются на клинте и исполняются, например скрипты в комментариях)
//        - отраженные(отправляются на сервер, например с той же query строкой, и возвращаются на клиент, где и запускаются)
//        - DOM based(так же передаются через например query, но не до ходят до сервера, а остаются на клиенте)
//  - CSRF: обход SAME ORIGIN политики, за счет отпраки формы, к которой автоматически приципляются куки от этого же домена,
//      в результате, выполняется действие от имени пользователя.
//  - CORS: Cross-origin resource sharing is a mechanism to safely bypass the same-origin policy.
