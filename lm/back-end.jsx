...
//          - В libuv по умоланию 4 потока\воркера, так что контекст будет перключатся
//          только между ними. Тогда как при обработке web запросов, потоков может быть 
//          гораздо больше. Поэтому тут тем не менее есть экономия в памяти, и времени
//          на перключение контекста.
//          - Плюс потоки асинхронного ввода\вывода обычно длятся продолжительное время
//          так что количество перключений тоже будет меньше.
